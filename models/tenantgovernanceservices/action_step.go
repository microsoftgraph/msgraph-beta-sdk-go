...
// GetActionUrl gets the actionUrl property value. The follow-on API reference for the step, containing the URL template and a machine-readable execution directive that a client uses to retrieve the drill-in data.
...
// GetStepNumber gets the stepNumber property value. The one-based order, as a string, in which the step should be evaluated by a client. Steps are intended to be run in ascending stepNumber order because later steps can depend on the output of earlier steps. This value is the key of the resource.
// returns a *string when successful
func (m *ActionStep) GetStepNumber()(*string) {
...
// GetText gets the text property value. Human-readable guidance that explains what the step does and why it's useful for investigating the related metric.
// returns a *string when successful
...
// SetActionUrl sets the actionUrl property value. The follow-on API reference for the step, containing the URL template and a machine-readable execution directive that a client uses to retrieve the drill-in data.
...
// SetStepNumber sets the stepNumber property value. The one-based order, as a string, in which the step should be evaluated by a client. Steps are intended to be run in ascending stepNumber order because later steps can depend on the output of earlier steps. This value is the key of the resource.
func (m *ActionStep) SetStepNumber(value *string)() {
    err := m.GetBackingStore().Set("stepNumber", value)
    if err != nil {
        panic(err)
    }
}
// SetText sets the text property value. Human-readable guidance that explains what the step does and why it's useful for investigating the related metric.
