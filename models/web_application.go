...
// SetLogoutUrl sets the logoutUrl property value. Specifies the URL that will be used by Microsoft's authorization service to logout a user using front-channel, back-channel or SAML logout protocols.
...
func (m *WebApplication) SetOauth2AllowImplicitFlow(value *bool)() {
    err := m.GetBackingStore().Set("oauth2AllowImplicitFlow", value)
    if err != nil {
        panic(err)
    }
}
