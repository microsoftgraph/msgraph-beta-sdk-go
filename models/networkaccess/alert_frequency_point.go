package networkaccess

import (
    i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e "time"
    i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91 "github.com/microsoft/kiota-abstractions-go/serialization"
    ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e "github.com/microsoft/kiota-abstractions-go/store"
)

type AlertFrequencyPoint struct {
    // Stores model information.
    backingStore ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStore
}
// NewAlertFrequencyPoint instantiates a new AlertFrequencyPoint and sets the default values.
func NewAlertFrequencyPoint()(*AlertFrequencyPoint) {
    m := &AlertFrequencyPoint{
    }
    m.backingStore = ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStoreFactoryInstance();
    m.SetAdditionalData(make(map[string]any))
    return m
}
// CreateAlertFrequencyPointFromDiscriminatorValue creates a new instance of the appropriate class based on discriminator value
// returns a Parsable when successful
func CreateAlertFrequencyPointFromDiscriminatorValue(parseNode i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.Parsable, error) {
    return NewAlertFrequencyPoint(), nil
}
// GetAdditionalData gets the AdditionalData property value. Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
// returns a map[string]any when successful
func (m *AlertFrequencyPoint) GetAdditionalData()(map[string]any) {
    val , err :=  m.backingStore.Get("additionalData")
    if err != nil {
        panic(err)
    }
    if val == nil {
        var value = make(map[string]any);
        m.SetAdditionalData(value);
    }
    return val.(map[string]any)
}
// GetBackingStore gets the BackingStore property value. Stores model information.
// returns a BackingStore when successful
func (m *AlertFrequencyPoint) GetBackingStore()(ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStore) {
    return m.backingStore
}
// GetFieldDeserializers the deserialization information for the current model
// returns a map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(error) when successful
func (m *AlertFrequencyPoint) GetFieldDeserializers()(map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(error)) {
    res := make(map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(error))
    res["highSeverityCount"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetInt64Value()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetHighSeverityCount(val)
        }
        return nil
    }
    res["informationalSeverityCount"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetInt64Value()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetInformationalSeverityCount(val)
        }
        return nil
    }
    res["lowSeverityCount"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetInt64Value()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetLowSeverityCount(val)
        }
        return nil
    }
    res["mediumSeverityCount"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetInt64Value()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetMediumSeverityCount(val)
        }
        return nil
    }
    res["@odata.type"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetStringValue()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetOdataType(val)
        }
        return nil
    }
    res["timeStampDateTime"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetTimeValue()
        if err != nil {
            return err
        }
        if val != nil {
            m.SetTimeStampDateTime(val)
        }
        return nil
    }
    return res
}
// GetHighSeverityCount gets the highSeverityCount property value. The highSeverityCount property
// returns a *int64 when successful
func (m *AlertFrequencyPoint) GetHighSeverityCount()(*int64) {
    val, err := m.GetBackingStore().Get("highSeverityCount")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*int64)
    }
    return nil
}
// GetInformationalSeverityCount gets the informationalSeverityCount property value. The informationalSeverityCount property
// returns a *int64 when successful
func (m *AlertFrequencyPoint) GetInformationalSeverityCount()(*int64) {
    val, err := m.GetBackingStore().Get("informationalSeverityCount")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*int64)
    }
    return nil
}
// GetLowSeverityCount gets the lowSeverityCount property value. The lowSeverityCount property
// returns a *int64 when successful
func (m *AlertFrequencyPoint) GetLowSeverityCount()(*int64) {
    val, err := m.GetBackingStore().Get("lowSeverityCount")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*int64)
    }
    return nil
}
// GetMediumSeverityCount gets the mediumSeverityCount property value. The mediumSeverityCount property
// returns a *int64 when successful
func (m *AlertFrequencyPoint) GetMediumSeverityCount()(*int64) {
    val, err := m.GetBackingStore().Get("mediumSeverityCount")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*int64)
    }
    return nil
}
// GetOdataType gets the @odata.type property value. The OdataType property
// returns a *string when successful
func (m *AlertFrequencyPoint) GetOdataType()(*string) {
    val, err := m.GetBackingStore().Get("odataType")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*string)
    }
    return nil
}
// GetTimeStampDateTime gets the timeStampDateTime property value. The timeStampDateTime property
// returns a *Time when successful
func (m *AlertFrequencyPoint) GetTimeStampDateTime()(*i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time) {
    val, err := m.GetBackingStore().Get("timeStampDateTime")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(*i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)
    }
    return nil
}
// Serialize serializes information the current object
func (m *AlertFrequencyPoint) Serialize(writer i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.SerializationWriter)(error) {
    {
        err := writer.WriteInt64Value("highSeverityCount", m.GetHighSeverityCount())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteInt64Value("informationalSeverityCount", m.GetInformationalSeverityCount())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteInt64Value("lowSeverityCount", m.GetLowSeverityCount())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteInt64Value("mediumSeverityCount", m.GetMediumSeverityCount())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteStringValue("@odata.type", m.GetOdataType())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteTimeValue("timeStampDateTime", m.GetTimeStampDateTime())
        if err != nil {
            return err
        }
    }
    {
        err := writer.WriteAdditionalData(m.GetAdditionalData())
        if err != nil {
            return err
        }
    }
    return nil
}
// SetAdditionalData sets the AdditionalData property value. Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
func (m *AlertFrequencyPoint) SetAdditionalData(value map[string]any)() {
    err := m.GetBackingStore().Set("additionalData", value)
    if err != nil {
        panic(err)
    }
}
// SetBackingStore sets the BackingStore property value. Stores model information.
func (m *AlertFrequencyPoint) SetBackingStore(value ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStore)() {
    m.backingStore = value
}
// SetHighSeverityCount sets the highSeverityCount property value. The highSeverityCount property
func (m *AlertFrequencyPoint) SetHighSeverityCount(value *int64)() {
    err := m.GetBackingStore().Set("highSeverityCount", value)
    if err != nil {
        panic(err)
    }
}
// SetInformationalSeverityCount sets the informationalSeverityCount property value. The informationalSeverityCount property
func (m *AlertFrequencyPoint) SetInformationalSeverityCount(value *int64)() {
    err := m.GetBackingStore().Set("informationalSeverityCount", value)
    if err != nil {
        panic(err)
    }
}
// SetLowSeverityCount sets the lowSeverityCount property value. The lowSeverityCount property
func (m *AlertFrequencyPoint) SetLowSeverityCount(value *int64)() {
    err := m.GetBackingStore().Set("lowSeverityCount", value)
    if err != nil {
        panic(err)
    }
}
// SetMediumSeverityCount sets the mediumSeverityCount property value. The mediumSeverityCount property
func (m *AlertFrequencyPoint) SetMediumSeverityCount(value *int64)() {
    err := m.GetBackingStore().Set("mediumSeverityCount", value)
    if err != nil {
        panic(err)
    }
}
// SetOdataType sets the @odata.type property value. The OdataType property
func (m *AlertFrequencyPoint) SetOdataType(value *string)() {
    err := m.GetBackingStore().Set("odataType", value)
    if err != nil {
        panic(err)
    }
}
// SetTimeStampDateTime sets the timeStampDateTime property value. The timeStampDateTime property
func (m *AlertFrequencyPoint) SetTimeStampDateTime(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)() {
    err := m.GetBackingStore().Set("timeStampDateTime", value)
    if err != nil {
        panic(err)
    }
}
type AlertFrequencyPointable interface {
    i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.AdditionalDataHolder
    ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackedModel
    i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.Parsable
    GetBackingStore()(ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStore)
    GetHighSeverityCount()(*int64)
    GetInformationalSeverityCount()(*int64)
    GetLowSeverityCount()(*int64)
    GetMediumSeverityCount()(*int64)
    GetOdataType()(*string)
    GetTimeStampDateTime()(*i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)
    SetBackingStore(value ie8677ce2c7e1b4c22e9c3827ecd078d41185424dd9eeb92b7d971ed2d49a392e.BackingStore)()
    SetHighSeverityCount(value *int64)()
    SetInformationalSeverityCount(value *int64)()
    SetLowSeverityCount(value *int64)()
    SetMediumSeverityCount(value *int64)()
    SetOdataType(value *string)()
    SetTimeStampDateTime(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)()
}
