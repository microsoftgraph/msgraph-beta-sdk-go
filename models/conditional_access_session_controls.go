...
// GetContinuousAccessEvaluation gets the continuousAccessEvaluation property value. Session control for continuous access evaluation settings.
// returns a ContinuousAccessEvaluationSessionControlable when successful
func (m *ConditionalAccessSessionControls) GetContinuousAccessEvaluation()(ContinuousAccessEvaluationSessionControlable) {
    val, err := m.GetBackingStore().Get("continuousAccessEvaluation")
    if err != nil {
        panic(err)
    }
    if val != nil {
        return val.(ContinuousAccessEvaluationSessionControlable)
    }
    return nil
}
// GetDisableResilienceDefaults gets the disableResilienceDefaults property value. Session control that determines whether it's acceptable for Microsoft Entra ID to extend existing sessions based on information collected prior to an outage or not.
...
func (m *ConditionalAccessSessionControls) GetFieldDeserializers()(map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(error)) {
    ...
    res["continuousAccessEvaluation"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        ...
        if val != nil {
            m.SetContinuousAccessEvaluation(val.(ContinuousAccessEvaluationSessionControlable))
        }
        return nil
    }
...
func (m *ConditionalAccessSessionControls) Serialize(writer i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.SerializationWriter)(error) {
    ...
    {
        err := writer.WriteObjectValue("continuousAccessEvaluation", m.GetContinuousAccessEvaluation())
        if err != nil {
            return err
        }
    }
...
// SetContinuousAccessEvaluation sets the continuousAccessEvaluation property value. Session control for continuous access evaluation settings.
func (m *ConditionalAccessSessionControls) SetContinuousAccessEvaluation(value ContinuousAccessEvaluationSessionControlable)() {
    err := m.GetBackingStore().Set("continuousAccessEvaluation", value)
    if err != nil {
        panic(err)
    }
}
// SetDisableResilienceDefaults sets the disableResilienceDefaults property value. Session control that determines whether it's acceptable for Microsoft Entra ID to extend existing sessions based on information collected prior to an outage or not.
