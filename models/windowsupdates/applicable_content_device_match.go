...
// GetRecommendedBy gets the recommendedBy property value. Collection of vendors who recommend the content.
// returns a []string when successful
func (m *ApplicableContentDeviceMatch) GetRecommendedBy()([]string) {
    val, err := m.GetBackingStore().Get("recommendedBy")
    if err != nil {
        panic(err)
    }
    if val != nil {
...
// SetRecommendedBy sets the recommendedBy property value. Collection of vendors who recommend the content.
func (m *ApplicableContentDeviceMatch) SetRecommendedBy(value []string)() {
    err := m.GetBackingStore().Set("recommendedBy", value)
    if err != nil {
        panic(err)
    }
}
