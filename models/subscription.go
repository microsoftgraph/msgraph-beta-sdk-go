...
func (m *Subscription) GetNotificationContentType()(*string) {
    val, err := m.GetBackingStore().Get("notificationContentType")
    if err != nil {
        panic(err)
    }
    if val != nil {
...
// GetResource gets the resource property value. Required. Specifies the resource that is monitored for changes. Don't include the base URL (https://graph.microsoft.com/beta/). See the possible resource path values for each supported resource.
// returns a *string when successful
...
func (m *Subscription) SetNotificationContentType(value *string)() {
    err := m.GetBackingStore().Set("notificationContentType", value)
    if err != nil {
        panic(err)
    }
}
...
// SetResource sets the resource property value. Required. Specifies the resource that is monitored for changes. Don't include the base URL (https://graph.microsoft.com/beta/). See the possible resource path values for each supported resource.
