...
// GetAppDisplayName gets the appDisplayName property value. The application name displayed in the Microsoft Entra admin center.  Supports $filter (eq, startsWith).
...
// GetAppId gets the appId property value. The application identifier in Microsoft Entra ID.  Supports $filter (eq).
// returns a *string when successful
...
// GetAppliedEventListeners gets the appliedEventListeners property value. Detailed information about the listeners, such as Azure Logic Apps and Azure Functions, which the corresponding events in the sign-in event triggered.
...
// GetAppOwnerTenantId gets the appOwnerTenantId property value. The identifier of the tenant that owns the client application.  Supports $filter (eq).
// returns a *string when successful
...
// GetAuthenticationAppDeviceDetails gets the authenticationAppDeviceDetails property value. Provides details about the app and device used during a Microsoft Entra authentication step.
...
// GetAuthenticationAppPolicyEvaluationDetails gets the authenticationAppPolicyEvaluationDetails property value. Provides details of the Microsoft Entra policies applied to a user and client authentication app during an authentication step.
// returns a []AuthenticationAppPolicyDetailsable when successful
...
// returns a []AuthenticationContextable when successful
...
// returns a []AuthenticationDetailable when successful
...
// returns a []AuthenticationRequirementPolicyable when successful
...
// returns a *ClientCredentialType when successful
...
// GetFlaggedForReview gets the flaggedForReview property value. During a failed sign-in, a user can select a button in the Azure portal to mark the failed event for tenant admins. If a user selects the button to flag the failed sign-in, this value is true.
...
// GetIsTenantRestricted gets the isTenantRestricted property value. Shows whether the sign in event was subject to a Microsoft Entra tenant restriction policy.
// returns a *bool when successful
...
// returns a []NetworkLocationDetailable when successful
...
// returns a *OriginalTransferMethods when successful
...
// GetPrivateLinkDetails gets the privateLinkDetails property value. Contains information about the Microsoft Entra Private Link policy that is associated with the sign in event.
// returns a PrivateLinkDetailsable when successful
...
// GetResourceOwnerTenantId gets the resourceOwnerTenantId property value. The identifier of the owner of the resource.  Supports $filter (eq).
// returns a *string when successful
...
// returns a []SessionLifetimePolicyable when successful
...
// GetSignInIdentifier gets the signInIdentifier property value. The identification that the user provided to sign in. It can be the userPrincipalName, but is also populated when a user signs in using other identifiers.
// returns a *string when successful
...
// GetTokenIssuerType gets the tokenIssuerType property value. The type of identity provider. The possible values are: AzureAD, ADFederationServices, UnknownFutureValue, AzureADBackupAuth, ADFederationServicesMFAAdapter, NPSExtension. Use the Prefer: include-unknown-enum-members request header to get the following values in this evolvable enum: AzureADBackupAuth , ADFederationServicesMFAAdapter , NPSExtension.
...
// SetAppDisplayName sets the appDisplayName property value. The application name displayed in the Microsoft Entra admin center.  Supports $filter (eq, startsWith).
...
// SetAppId sets the appId property value. The application identifier in Microsoft Entra ID.  Supports $filter (eq).
...
// SetAppOwnerTenantId sets the appOwnerTenantId property value. The identifier of the tenant that owns the client application.  Supports $filter (eq).
...
// SetAuthenticationAppDeviceDetails sets the authenticationAppDeviceDetails property value. Provides details about the app and device used during a Microsoft Entra authentication step.
...
// SetAuthenticationAppPolicyEvaluationDetails sets the authenticationAppPolicyEvaluationDetails property value. Provides details of the Microsoft Entra policies applied to a user and client authentication app during an authentication step.
...
// SetFlaggedForReview sets the flaggedForReview property value. During a failed sign-in, a user can select a button in the Azure portal to mark the failed event for tenant admins. If a user selects the button to flag the failed sign-in, this value is true.
...
// SetPrivateLinkDetails sets the privateLinkDetails property value. Contains information about the Microsoft Entra Private Link policy that is associated with the sign in event.
...
// SetResourceOwnerTenantId sets the resourceOwnerTenantId property value. The identifier of the owner of the resource.  Supports $filter (eq).
...
// SetSignInIdentifier sets the signInIdentifier property value. The identification that the user provided to sign in. It can be the userPrincipalName, but is also populated when a user signs in using other identifiers.
