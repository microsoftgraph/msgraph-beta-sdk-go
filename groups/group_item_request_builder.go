...
func (m *GroupItemRequestBuilder) EndpointsById(id string)(*EndpointItemRequestBuilder) {
    urlTplParams := make(map[string]string)
    for idx, item := range m.pathParameters {
        urlTplParams[idx] = item
    }
...
func (m *GroupItemRequestBuilder) SettingsById(id string)(*DirectorySettingItemRequestBuilder) {
    urlTplParams := make(map[string]string)
    for idx, item := range m.pathParameters {
        urlTplParams[idx] = item
    }
    ...
    return NewDirectorySettingItemRequestBuilderInternal(urlTplParams, m.requestAdapter);
}
