...
// 
// [Find more info here]: https://learn.microsoft.com/graph/api/directoryobject-getbyids?view=graph-rest-1.0
func (m *ItemJoinedGroupsGetByIdsRequestBuilder) PostAsGetByIdsPostResponse(ctx context.Context, body ItemJoinedGroupsGetByIdsPostRequestBodyable, requestConfiguration *ItemJoinedGroupsGetByIdsRequestBuilderPostRequestConfiguration)(ItemJoinedGroupsGetByIdsPostResponseable, error) {
    requestInfo, err := m.ToPostRequestInformation(ctx, body, requestConfiguration);
    if err != nil {
        return nil, err
    }
    ...
    res, err := m.BaseRequestBuilder.RequestAdapter.Send(ctx, requestInfo, CreateItemJoinedGroupsGetByIdsPostResponseFromDiscriminatorValue, errorMapping)
    if err != nil {
        return nil, err
    }
    if res == nil {
    ...
    return res.(ItemJoinedGroupsGetByIdsPostResponseable), nil
}
