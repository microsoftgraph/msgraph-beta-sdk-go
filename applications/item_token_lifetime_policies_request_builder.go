...
// ItemTokenLifetimePoliciesRequestBuilderGetQueryParameters list the tokenLifetimePolicy objects that are assigned to an application or servicePrincipal. Only one object is returned in the collection because only one tokenLifetimePolicy can be assigned to an application.
...
// ByTokenLifetimePolicyIdString gets an item from the github.com/microsoftgraph/msgraph-beta-sdk-go/.applications.item.tokenLifetimePolicies.item collection
