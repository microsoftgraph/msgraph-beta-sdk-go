...
func (m *SetScheduledActionsRequestBuilder) Post(options *SetScheduledActionsRequestBuilderPostOptions)([]SetScheduledActions, error) {
    ...
    res, err := m.requestAdapter.SendCollectionAsync(*requestInfo, func () i04eb5309aeaafadd28374d79c8471df9b267510b4dc2e3144c378c50f6fd7b55.Parsable { return NewSetScheduledActions() }, nil, nil)
    if err != nil {
        return nil, err
    }
    val := make([]SetScheduledActions, len(res))
    for i, v := range res {
        val[i] = *(v.(*SetScheduledActions))
    }
    return val, nil
}
