...
func (m *ItemScheduleRequestBuilder) TimeCards()(*ItemScheduleTimeCardsRequestBuilder) {
    return NewItemScheduleTimeCardsRequestBuilderInternal(m.pathParameters, m.requestAdapter)
}
// TimeCardsById provides operations to manage the timeCards property of the microsoft.graph.schedule entity.
func (m *ItemScheduleRequestBuilder) TimeCardsById(id string)(*ItemScheduleTimeCardsTimeCardItemRequestBuilder) {
    urlTplParams := make(map[string]string)
    for idx, item := range m.pathParameters {
        urlTplParams[idx] = item
    }
    idPtr := &id
    return NewItemScheduleTimeCardsTimeCardItemRequestBuilderInternal(urlTplParams, m.requestAdapter, idPtr)
}
