...
// DeltaWithTokenRequestBuilder builds and executes requests for operations under \sites\{site-id}\lists\{list-id}\items\microsoft.graph.delta(token='{token}')
...
func NewDeltaWithTokenRequestBuilderInternal(pathParameters map[string]string, requestAdapter ida96af0f171bb75f894a4013a6b3146a4397c58f11adb81a2b7cbea9314783a9.RequestAdapter, token *string)(*DeltaWithTokenRequestBuilder) {
    ...
    m.urlTemplate = "{+baseurl}/sites/{site_id}/lists/{list_id}/items/microsoft.graph.delta(token='{token}')";
