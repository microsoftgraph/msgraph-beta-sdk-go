...
// PlansItemBucketsItemTasksDeltaRequestBuilderGetQueryParameters get newly created, updated, or deleted tasks in either a Planner plan or assigned to the signed-in user without having to perform a full read of the entire resource collection. For details, see Use delta query to track changes in Microsoft Graph data.
...
// NewPlansItemBucketsItemTasksDeltaRequestBuilderInternal instantiates a new PlansItemBucketsItemTasksDeltaRequestBuilder and sets the default values.
func NewPlansItemBucketsItemTasksDeltaRequestBuilderInternal(pathParameters map[string]string, requestAdapter i2ae4187f7daee263371cb1c977df639813ab50ffa529013b7437480d1ec0158f.RequestAdapter)(*PlansItemBucketsItemTasksDeltaRequestBuilder) {
    m := &PlansItemBucketsItemTasksDeltaRequestBuilder{
        BaseRequestBuilder: *i2ae4187f7daee263371cb1c977df639813ab50ffa529013b7437480d1ec0158f.NewBaseRequestBuilder(requestAdapter, "{+baseurl}/planner/plans/{plannerPlan%2Did}/buckets/{plannerBucket%2Did}/tasks/delta(){?%24count,%24expand,%24filter,%24orderby,%24search,%24select,%24skip,%24top}", pathParameters),
    }
    return m
}
// NewPlansItemBucketsItemTasksDeltaRequestBuilder instantiates a new PlansItemBucketsItemTasksDeltaRequestBuilder and sets the default values.
...
// returns a PlansItemBucketsItemTasksDeltaResponseable when successful
...
// returns a PlansItemBucketsItemTasksDeltaGetResponseable when successful
// returns a ODataError error when the service returns a 4XX or 5XX status code
// [Find more info here]
// 
// [Find more info here]: https://learn.microsoft.com/graph/api/plannertask-delta?view=graph-rest-beta
func (m *PlansItemBucketsItemTasksDeltaRequestBuilder) GetAsDeltaGetResponse(ctx context.Context, requestConfiguration *PlansItemBucketsItemTasksDeltaRequestBuilderGetRequestConfiguration)(PlansItemBucketsItemTasksDeltaGetResponseable, error) {
    requestInfo, err := m.ToGetRequestInformation(ctx, requestConfiguration);
    if err != nil {
        return nil, err
    }
    ...
    res, err := m.BaseRequestBuilder.RequestAdapter.Send(ctx, requestInfo, CreatePlansItemBucketsItemTasksDeltaGetResponseFromDiscriminatorValue, errorMapping)
    if err != nil {
        return nil, err
    }
    if res == nil {
    ...
    return res.(PlansItemBucketsItemTasksDeltaGetResponseable), nil
}
...
// returns a *PlansItemBucketsItemTasksDeltaRequestBuilder when successful
